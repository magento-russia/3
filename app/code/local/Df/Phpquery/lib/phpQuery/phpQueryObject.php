						...
						// ATTRS
							...
							if (mb_strpos($attr, '=')) {
								...
								if ($attr == 'nodeType') {
									...
									// cut last character
									$attr = substr($attr, 0, -1);
									$isMatch = preg_match("#{$pattern}#u", $node->getAttribute($attr));
									if (! $isMatch)
										$break = true;
...
//		$this->previous->XPath = $this->XPath;
	...
	public function replaceWith($content) {
		df_assert(
			false, 'Метод — нерабочий. Для документов HTML используйте вместо этого метода метод html().'
		);
