 ...
 * Пример применения:
 * @used-by \Df\C1\Cml2\Export\Processor\Catalog\Product::getElement_Производитель()
...
function df_column($collection, $methodForValue, $methodForKey = null) {
	...
	foreach ($collection as $id => $object) {
		/** @var int|string $id */
		/** @var Varien_Object|callable $object */
		/** @var int|string $key */
 ...
 * @param \Traversable|array(int|string => Varien_Object) $items
 * @return mixed[]
 */
 ...
 * @return void
 */
function dfa_deep_walk(&$node, $path, $callback, $params = null) {
	if (!is_array($node)) {
		if (!$path) {
		...
		while ($path) {
			...
			if ('*' !== $step) {
				if (isset($node[$step])) {
					...
					dfa_deep_walk($node[$step], $path, $callback, $params);
				}
			}
			else if ($path) {
				...
				foreach ($node as &$child) {
					/** @var mixed $child */
					dfa_deep_walk($child, $path, $callback, $params);
				}
			}
		}
	}
