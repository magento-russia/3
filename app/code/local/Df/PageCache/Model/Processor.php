<?php
class Df_PageCache_Model_Processor
{
	...
	const XML_PATH_CACHE_DEBUG          = 'df_speed/page_cache/debug';
	...
	const XML_PATH_CACHE_MAX_SIZE       = 'df_speed/page_cache/max_cache_size';
	...
	protected function _createRequestIds()
	{
		...
		if ($uri) {
			...
			if (isset($_COOKIE[Df_PageCache_Model_Cookie::COOKIE_CUSTOMER_GROUP])) {
				$uri .= '_' . $_COOKIE[Df_PageCache_Model_Cookie::COOKIE_CUSTOMER_GROUP];
			}
			if (isset($_COOKIE[Df_PageCache_Model_Cookie::COOKIE_CUSTOMER_LOGGED_IN])) {
				$uri .= '_' . $_COOKIE[Df_PageCache_Model_Cookie::COOKIE_CUSTOMER_LOGGED_IN];
			}
			if (isset($_COOKIE[Df_PageCache_Model_Cookie::CUSTOMER_SEGMENT_IDS])) {
				$uri .= '_' . $_COOKIE[Df_PageCache_Model_Cookie::CUSTOMER_SEGMENT_IDS];
			}
			if (isset($_COOKIE[Df_PageCache_Model_Cookie::IS_USER_ALLOWED_SAVE_COOKIE])) {
				$uri .= '_' . $_COOKIE[Df_PageCache_Model_Cookie::IS_USER_ALLOWED_SAVE_COOKIE];
			}
			if (Df_PageCache_Helper_Data::isSSL()) {
				$uri .= '_ssl';
			}
		...
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} =
				...
				||
					isset($_GET['no_cache'])
				||
					isset($_GET[Mage_Core_Model_Session_Abstract::SESSION_ID_QUERY_PARAM])
				||
				...
				if ($requestUriExploded) {
					 ...
					 * @see Df_PageCache_Observer::processPreDispatch():
							if (
									$request->isPost()
								||
									in_array($controller->getFullActionName(), array(
										'checkout_cart_add','catalog_product_compare_add'
									))
							)
					 * В частности, на странице удобного оформления заказа
					 * полностраничное кэширование раньше отключалось
					 * после асинхронного запроса /checkout/onepage/saveBilling/
					 * Получалось так:
						1) /checkout/onepage/  (полностраничное кэширование ещё включено)
						2) /checkout/onepage/saveBilling/ (полностраничное кэширование ещё включено)
						3) /checkout/onepage/saveShippingMethod/ (полностраничное кэширование отключено)
					 * Уж лучше его отключить сразу!
					 */
					$noCachePaths = array('df-1c/cml2', 'df-yandex-market/yml', '/checkout/onepage/');
	...
	protected function _processContent($content)
	{
		...
		$sessionInfo = Df_PageCache_Model_Cache::getCacheInstance()->load($this->getSessionInfoCacheId());
		...
		if (isset($_COOKIE[Df_PageCache_Model_Cookie::COOKIE_FORM_KEY])) {
			$formKey = $_COOKIE[Df_PageCache_Model_Cookie::COOKIE_FORM_KEY];
		} else {
			$formKey = Df_PageCache_Helper_Data::getRandomString(16);
			Df_PageCache_Model_Cookie::setFormKeyCookieValue($formKey);
		}

		Df_PageCache_Helper_Form_Key::restoreFormKey($content, $formKey);
		...
		Df_PageCache_Helper_Url::restoreSid($content, htmlspecialchars($sidCookieValue, ENT_QUOTES));
	...
	public function processRequestResponse(Zend_Controller_Request_Http $request,
		...
		$this->addRequestTag(Df_PageCache_Helper_Url::prepareRequestPathTag($request->getOriginalPathInfo()));
		$cacheInstance = Df_PageCache_Model_Cache::getCacheInstance();
		/**
	 ...
	 *
	 * @param Zend_Controller_Request_Http $request
	 * @return Df_PageCache_Model_Processor_Default|false
	 */
	...
	{
		Df_PageCache_Model_Cache::getCacheInstance()->save(
	...
	protected function _loadMetadata()
	{
		if ($this->_metaData === null) {
			$cacheMetadata = Df_PageCache_Model_Cache::getCacheInstance()
