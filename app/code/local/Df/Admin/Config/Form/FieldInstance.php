 ...
 */
class Df_Admin_Config_Form_FieldInstance extends Df_Core_Model {
	/**
	 * @param string $name
	...
	public function getConfigParam($name, $required = false, $defaultValue = null) {
		...
		if (is_null($result)) {
			if ($required) {
				df_error('Требуется непустое значение для параметра «%s».', $this->getConfigPath($name));
			}
			else {
				$result = $defaultValue;
			}
	...
	/** @return Df_Admin_Block_Field_Custom */
