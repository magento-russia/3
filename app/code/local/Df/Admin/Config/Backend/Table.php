 ...
 */
class Df_Admin_Config_Backend_Table extends Df_Admin_Config_Backend {
	/**
	 * 2015-04-17
	 * @override
	 * @see Mage_Core_Model_Abstract::_afterLoad()
	 * @used-by Mage_Core_Model_Config_Data::afterLoad()
	 * @used-by Mage_Core_Model_Abstract::load()
	 * Метод реализован по аналогии с
	 * @see Mage_Adminhtml_Model_System_Config_Backend_Serialized::_afterLoad()
	...
	public static function unserialize($value, $rowClass) {
		...
		if (!df_contains($value, array('"', '[', '{'))) {
			...
			/**
			 * Для обратной совместимости с элементами управления, которые ранее использовали
			 * @see Mage_Adminhtml_Model_System_Config_Backend_Serialized,
			 * а затем перешли на @see Df_Admin_Config_Backend_Table,
			 * пробуем распаковать данные посредством @see unserialize().
			 */
			if (is_null($result)) {
		...
		if (is_array($result)) {
			...
			/**
			 * @uses \Df\C1\Config\MapItem\CurrencyCode::fields()
			 * @uses \Df\C1\Config\MapItem\PriceType::fields()
