...
class Api extends \Df_Core_Model {
	...
	private function call(
		...
		/** @var mixed $response */
		$response = df_stdclass_to_array(
			$this->_soap->$method(array($requestKey => $params))->return
		);
		/** @var string $state */
		$state = dfa($response, 'InterfaceState');
		if ('OK' !== $state) {
			unset($params[$authKey]);
			df_error(new \Df\Core\Exception("[{$method}] {$state}", df_print_params($params)));
		}
		/** @var mixed[] $result */
		$result =
			'' === $responseKey
			? $response
			: dfa_deep($response, $responseKey ? $responseKey : $method)
		;
