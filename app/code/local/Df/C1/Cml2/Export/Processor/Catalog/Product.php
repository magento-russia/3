...
class Product extends \Df_Catalog_Model_XmlExport_Product {
	...
	private function getOutput_ЗначенияРеквизитов() {return [[
		'ЗначениеРеквизита' => $this->entry()->name(
	...
	private function getOutput_ЗначенияСвойств() {
		...
		foreach ($this->getDocument()->getCatalogAttributes() as $attribute) {
			...
			$processor = $this->getDocument()->processorForAttribute($attribute);
		...
		return df_clean_xml(['ЗначенияСвойства' => df_clean($attributeValues)]);
	}

	/** @return array(string => string) */
	private function getOutput_ИдентификаторТовара() {return df_clean_xml([
		'Ид' => $this->getExternalId()
		,'Штрихкод' => ''
		,'Артикул' => $this->getProduct()->getSku()
	]);}
	...
	/** @return array(string => string) */
	private function getOutput_Производитель() {return df_clean_xml([
	...
	private function getExternalId() {return dfc($this, function() {
		...
		return $this->getProduct()->get1CId();
	});}

	/** @return string|null */
	private function getВес() {return
